...
> **Status: Phase 4b complete (Omarchy shell plugin).** Lifecycle, port
> allocation, persistence, drift reconciliation, restart policies, reboot
> recovery, the TUI and both status bar widgets are implemented, unit tested,
> and verified end to end against real rootless Podman containers. See
> [Roadmap](#roadmap).
...
This is the only install path today; the AUR package below is built and tested
...
`make pkgbuild-check` builds it and verifies what it installs. It is **not yet
on the AUR** — its `source` still names the original repository, so it needs
repointing at this one before it would build for anyone else.
...
There are two, because Omarchy has two bars. Pick the one matching your
install; they are alternatives, not companions.

| Your Omarchy | Use | Install with |
|---|---|---|
| 4.0 or newer (Quickshell shell) | the shell plugin | `omarchy plugin add` |
| 3.x, or a waybar setup | the waybar module | `./packaging/waybar/install.sh` |
...
A native `bar-widget` plugin, built to the [Omarchy plugin
guide](https://plugins.omarchy.org/develop.html). The bar shows a drawn
database mark, lit when something is running and urgent when an instance needs
attention; clicking it opens a keyboard-driven panel with one row per instance,
a switch to start or stop it, and a button to copy its connection string.

Creating and destroying instances is deliberately absent from the panel: both
are destructive enough to deserve a confirmation step, and that is the TUI's
job. `t`, or a middle click on the bar, gets you there.

**The widget lives at the root of this repository, and DBForge under
`dbforge/`.** That arrangement is forced, not chosen: Omarchy installs a plugin
by cloning its repository and reading `manifest.json` at the *root*, so the
plugin cannot sit in a subdirectory. Since the clone becomes the plugin folder,
installing the widget also drops this Go source into
`~/.config/omarchy/plugins/` — 804K, inert, and the price of one repository.

Keybindings, settings and the rest are in the [root
README](../README.md).

Documentation, keybindings and settings are in that repository's README. Why it
polls rather than using signals, and why the panel has no create/destroy, is in
[`docs/phase-4b-notes.md`](docs/phase-4b-notes.md).
...
sits at the repository root, because Omarchy requires the plugin manifest to be
there.
...
`pgrep -x waybar`. The Omarchy shell plugin does not use signals at all; it
polls on its own interval and refreshes whenever you open the panel.

**The Omarchy widget says "dbctl not found"** — same PATH trap as waybar, for
the same reason: the shell is started by the systemd user manager, which never
sources `~/.bashrc`. The plugin searches `~/.local/bin` and the usual prefixes
before `PATH`, so this means the binary is genuinely somewhere else. Point the
widget's `dbctl` setting at it, or set `DBFORGE_DBCTL`.

**The Omarchy widget never appears after installing** — the running shell has
already scanned its plugin directory. `omarchy-shell shell rescanPlugins` asks
it to look again; `omarchy plugin list --json` confirms it was discovered.
...
| 4b | Omarchy 4.0 shell plugin | ✅ done |
